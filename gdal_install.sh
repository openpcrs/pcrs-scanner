...
if [[ ! -e "$INSTALL_LOCKFILE" ]]; then
  ...
  # install gdal-async with special flags for scalingo
  CXXFLAGS="-I$HOME/.apt/usr/include/gdal" npm_config_build_from_source=true npm_config_shared_gdal=true yarn add --frozen-lockfile gdal-async
  rm "$INSTALL_LOCKFILE"
fi
