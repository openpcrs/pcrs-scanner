...
import {saveTreeItems, dropPreviousTreeItems, updateTreeItemComputedMetadata, updateTreeItemAnalyzeError, findDataItemsByScan} from './models/tree-item.js'
...
import {computeMetadata, computeRasterResult} from './raster.js'
...
export async function scan(storage) {
  ...
  try {
    ...
    const computedDataItems = await findDataItemsByScan({
      _scan: processingSince,
      _storage: _id
    })

    const rasterResult = computeRasterResult(computedDataItems)

    const duration = Math.round((Date.now() - processingSince.getTime()) / 1000)
    const result = {lastSuccessfulScan: processingSince, ...progress, duration, raster: rasterResult}
