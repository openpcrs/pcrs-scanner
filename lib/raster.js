...
function computeProjection(rasterFiles) {
  ...
  if (rasterFiles.every(({computedMetadata: {projection}}) => !projection || hashObject(projection) === projectionHash)) {
...
export function computeRasterResult(dataItems) {
  ...
  const projection = computeProjection(rasterFiles)
