...
import {computeDownloadToken} from '../util/security.js'
...
async function listItems(path, {client, rateLimit}) {
  ...
  return Promise.all(files.map(async file => {
    ...
    const item = {
      ...
      fullPath: sanitizePath(file.type === 'd' ? fullPath + '/' : fullPath)
    }
...
export function getCurlPath(params) {
  ...
  const token = computeDownloadToken({storage: storageId})

  return `${ROOT_URL}/storages/${storageId}/files${path}?token=${token}`
}
