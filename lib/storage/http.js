...
async function extractLinks(url, {rateLimit, signal}) {
  const response = await rateLimit(() => got.get(url, {signal}))
...
async function getFileMetadata(url, {rateLimit, signal}) {
  const response = await rateLimit(() => got.head(url, {signal}))
...
async function buildTree(url, {rateLimit, signal}) {
  const tree = []
  const links = await extractLinks(url, {rateLimit, signal})
  ...
  await Promise.all(links.map(async link => {
    ...
    if (isDirectory) {
      node.children = await buildTree(fullPath, {rateLimit, signal})
    } else {
      const metadata = await getFileMetadata(fullPath, {rateLimit, signal})
...
export async function listFiles(options = {}) {
  ...
  return buildTree(url, {rateLimit, signal: ac.signal})
}
