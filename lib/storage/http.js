...
export async function downloadFile(storage, path, req, res) {
  if (storage.type !== 'http') {
    throw new Error('Storage type mismatch')
  }

  if (storage.isPublic) {
    // Redirection to public file
    res.setHeader('location', path)
    res.sendStatus(301)
  } else {
    try {
      const client = getClient({url: storage.params.url})
      const rateLimit = pLimit(1)
      const {size} = computeFileMetadata(path, {rateLimit, client})
      ...
      const readStream = got.stream(path)
